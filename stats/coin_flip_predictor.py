...
class FlipPredictor(object):
    ...
    #the probability of the next flip being heads
        prob=sum([a*b for a,b in zip(self.coins,self.probs)])
        return prob
    ...
    #the probabilities of flipping each coin
        if result == 'H':
            p_heads=sum([a*b for a,b in zip(self.coins,self.probs)])
            self.probs=[a*b/p_heads for a,b in zip(self.coins,self.probs)]

        #print(p_heads)
        else:
            p_tails=sum([(1-a)*b for a,b in zip(self.coins,self.probs)])
            self.probs=[(1-a)*b/p_tails for a,b in zip(self.coins,self.probs)]
